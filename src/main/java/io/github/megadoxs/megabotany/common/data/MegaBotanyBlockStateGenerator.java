...
public class MegaBotanyBlockStateGenerator extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        var spiritPortalModel = models().cubeAll(name(MegaBotanyBlocks.SPIRIT_PORTAL.get()), blockTexture(MegaBotanyBlocks.SPIRIT_PORTAL.get()));
        var spiritPortalActivatedModel = models().cubeAll(name(MegaBotanyBlocks.SPIRIT_PORTAL.get()) + "_activated", modLoc("block/" + name(MegaBotanyBlocks.SPIRIT_PORTAL.get()) + "_activated"));

        getVariantBuilder(MegaBotanyBlocks.SPIRIT_PORTAL.get()).forAllStates(state -> {
            ConfiguredModel.Builder builder = ConfiguredModel.builder();
            AlfheimPortalState portalState = state.getValue(BotaniaStateProperties.ALFPORTAL_STATE);
            if (portalState == AlfheimPortalState.OFF) {
                builder.modelFile(spiritPortalModel);
            } else {
                builder.modelFile(spiritPortalActivatedModel);
            }
            return builder.build();
        });

        simpleBlockItem(MegaBotanyBlocks.SPIRIT_PORTAL.get(), spiritPortalModel);
    }
