...
public class ScaledDrawableResource implements IDrawableStatic {
    ...
    public ScaledDrawableResource(ResourceLocation resourceLocation, int u, int v, int width, int height, int paddingTop, int paddingBottom, int paddingLeft, int paddingRight, int textureWidth, int textureHeight) {
    ...
    public void draw(GuiGraphics guiGraphics, int xOffset, int yOffset, int maskTop, int maskBottom, int maskLeft, int maskRight) {
        ...
        bufferbuilder.vertex(matrix, x, y + height, 0).uv(u * f, (v + (float) textureHeight) * f1).endVertex();
        bufferbuilder.vertex(matrix, x + width, y + height, 0).uv((u + (float) textureWidth) * f, (v + (float) textureHeight) * f1).endVertex();
        bufferbuilder.vertex(matrix, x + width, y, 0).uv((u + (float) textureWidth) * f, v * f1).endVertex();
