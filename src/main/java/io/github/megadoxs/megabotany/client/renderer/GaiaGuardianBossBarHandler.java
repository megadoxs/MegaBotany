...
public final class GaiaGuardianBossBarHandler {

    private GaiaGuardianBossBarHandler() {
    }

    // Only access on the client thread!
    public static final Set<GaiaGuardianIII> bosses = Collections.newSetFromMap(new WeakHashMap<>());
    ...
    public static OptionalInt onBarRender(GuiGraphics gui, int x, int y, BossEvent bossEvent, boolean drawName) {
        for (GaiaGuardianIII currentBoss : bosses) {
            if (currentBoss.getBossInfoUuid().equals(bossEvent.getId())) {
                ...
                int mobs = currentBoss.getSpawnedMob();
                if (mobs > 0)
                    drawMobCount(mobs, gui, x, y);
    ...
    private static void drawMobCount(int mobCount, GuiGraphics gui, int x, int y) {
        PoseStack ps = gui.pose();
        ps.pushPose();
        int px = x + 137;
        int py = y + 12;
        ...
        gui.drawString(mc.font, Integer.toString(mobCount), px + 15, py + 4, 0xFFFFFF);
        ps.popPose();
    }

    private static void drawHealthBar(GuiGraphics gui, GaiaGuardianIII currentBoss, int x, int y, int u, int v, int w, int h, boolean bg) {
        var shader = CoreShaders.dopplegangerBar();
        if (shader != null) {
            float time = currentBoss.getInvulnerableTime();
